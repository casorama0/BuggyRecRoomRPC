...
while not initialized:
  ...
  try:
    ...
    try:
      userID = requests.get(
        f"https://accounts.rec.net/account?username={username}").json(
        )["accountId"]
    except:
...
while True:
  try:
    mmdata = requests.get(f"https://match.rec.net/player?id={user}",
                          headers={
                            "Authorization": token
                          }).json()[0]
    instdata = mmdata["roomInstance"]
    mm = Matchmaking(mmdata["isOnline"], instdata["isFull"],
                     instdata["isPrivate"], instdata["name"],
                     instdata["roomId"], instdata["roomInstanceType"],
                     instdata["isInProgress"])
  except:
    raise SystemExit(
      "Failed to gather instance data. Are you logged in to Rec Room?")
  ...
  try:
    RPC.update(state=f"Playing {rd.name}{priv}{isprogress}",
               large_image=f"https://img.rec.net/{rd.image}",
               start=timeOfChange)
  except:
